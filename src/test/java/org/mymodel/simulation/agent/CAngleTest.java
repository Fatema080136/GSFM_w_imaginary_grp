...
import org.mysimulationmodel.Genetic_Algorithm;
import org.mysimulationmodel.simulation.common.CForce;
import org.mysimulationmodel.simulation.common.CJsonObjectGenerator;
...
public class CAngleTest
{
    ...
    {
        CJsonObjectGenerator l_bla = new CJsonObjectGenerator();
        l_bla.check( 2, "ped3",23, 27);
        l_bla.check( 3, "ped3",26, 30);
        System.out.println( l_bla.getJsonObject() );
        Vector2d velocity1 = new Vector2d( -1, 0);
        Vector2d velocity2 = new Vector2d( 0.3, -0.7);
        Vector2d position = new Vector2d( 20, 200);
        Vector2d position2 = new Vector2d( 40, 220);
        double l_x = position.x + ( -1 * ( velocity1.x/velocity1.x ) );
        double l_y = position.y + ( -1 * ( velocity1.y/velocity1.y ) );
